...
public class ApiGeneratorTest {
    ...
    @Test
    public void testFilePayload2() throws RamlSpecException {
        RamlModelResult ramlModel;
        ...
        Spec spec = new ApiGeneratorPhp("org.generated").generate(ramlModel);
        ValueSpec valueSpec = this.getValueSpec(spec, "FileWithContentTypePostRequest");
        assertThat( valueSpec.propertySpecs().stream().filter(prop -> prop.name().equals("contentType")).count(), is(1L));

        valueSpec = this.getValueSpec(spec, "FileWithContentTypePostResponse");
        assertThat(valueSpec.propertySpec("status200").typeSpec().embeddedValueSpec().propertySpecs().stream().filter(prop -> prop.name().equals("contentType")).count(), is(1L));
    }
